...
sub _handle_receive_keepalive_message
{
    ...
    # If we do a state transition to established, we should
    # execute the callback.  We don't execute on an invalid state
    # transition.
    my $state = $this->{_fsm_state};
    if ( $state == BGP_STATE_OPEN_CONFIRM ) {

        # We want to establish the state in the object, even though this
        # violates the FSM somewhat, because we want to be able to UPDATE
        # and other things from both the KEEPALIVE and ESTABLISHED callbacks.
        $this->{_fsm_state} = BGP_STATE_ESTABLISHED;

        # Of course we just transitioned, so...
